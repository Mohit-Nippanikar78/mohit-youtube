...
export default function Header(props) {
  useEffect(() => {
    if (props.searchText === "") {
      ...
      for (let i = 0; i < props.videos.length; i++) {
        if (props.videos[i].title === props.searchText) {
          props.setsuggestions(false);
          break;
        } else {
          props.setsuggestions(true);
        }
      }
    }
  }, [props.searchText]);
  ...
  const speechToText = () => {
    document.getElementsByClassName("fa-microphone")[0].style.backgroundColor =
      "pink";
    ...
    recognition.onresult = function (event) {
      props.setSearchText(event.results[0][0].transcript);
    };
    recognition.start();
  };
  ...
  return (
    <header id="header">
      ...
      <div className="head2">
        <div className="head2-inner" style={{ width: "100%" }}>
          <div className="head21">
            <div className="head21-input">
              <input
                autoComplete="off"
                type="text"
            ...
            <div className="head21-icons">
              <Link to="/search">
                <i
                  id="head21-searchbtn"
                  className="fa fa-search head21-searchbtn"
                  onClick={() => {
                    console.log(props.searchText);
                  }}
                  // onKeyDown={(event)=>{if(event.keyCode === 13 ){    document.getElementById("head21-searchbtn").click()  }}}
                ></i>
              </Link>

              <i
                className="fa fa-microphone head21-microphone"
                onClick={() => {
                  speechToText();
                }}
              ></i>
      ...
      <div className="head3">
        <ul>
          <li>
            <Link to="/upload">
              <i className="fas fa-video" style={{ color: "#FFDB69" }}></i>
            </Link>
          </li>
          <li>
            <a href="/">
          ...
          <li>
            <img
              onClick={() => {
                props.setAccInfo(!props.accInfo);
              }}
              className="accphoto"
              src="https://d2v9ipibika81v.cloudfront.net/uploads/sites/165/President-Trump-Official-Portrait-scaled-e1591897715552.jpg"
              alt=""
            />
          </li>
        </ul>
